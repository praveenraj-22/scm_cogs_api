...
exports.othersMain = async (cogs, revenue, individual, group, branches, ftddate, vobres, breakupres, breakupmtdres) => {
  ...
  groupBranchArr.forEach(group => {
    ...
    group.forEach(branch => {
      let tempFiltercogs = _.filter(cogs, {
        branch: branch,
        trans_date: ftddate
      })
  ...
  individual.forEach(branch => {
    _.filter(branches, {
      code: branch
    }).forEach(ele => branchname = ele.branch)
    if (!shouldRemove.includes(branch)) {
      ...
      let tempFiltervob = _.filter(vobres, {
        billed: branch,
        trans_date: ftddate
      })
      if (tempFiltervob.length !== 0) {
...
let overseasCurrencyConversion = async (ftddate, currencyres, currencylastres) => {
  ...
  if ((currencyres.length > 0) && (ftddate >= '2019-11-01')) {
    ...
    if ((Object.keys(overseastempObj).length) > 0) {
      let mozamarr = _.filter(currencyres, {
        country_code: 'MZN'
      })
      _.filter(mozamarr, {
        currency_date: ftddate
      ...
      let mauritiusarr = _.filter(currencyres, {
        country_code: 'MUR'
      })
      _.filter(mauritiusarr, {
        currency_date: ftddate
      }).forEach(element => {
...
let filterEntity = async (dbres, dbres2, ftddate, overseasCurrency) => {
  ...
  tempObj.AEH = {
    ...
    ftd: total,
    ftdrev: rev,
    ftd_cogs_percent: cogsPercent(total, rev)
  }
  ...
  tempObj.OHC = {
    ...
    ftd: ohcftdtotal,
    ftdrev: ohcftdrev,
    ftd_cogs_percent: cogsPercent(ohcftdtotal, ohcftdrev)
  }
...
let filterGroupwise = async (aeh, ahc, ohc, dbres2, branches, ftddate, vobres, breakupres, breakupmtdres, overseasCurrency) => {
  ...
  let ahcgroupedBranches = {
    ...
    'Tiruppur': ["TPR"],
    'Andaman': ["AMN"],
    'Karnataka': ["BMH", "JGN", "WFD", "KML", "CLR", "INR", "PNR", "YLK", "SVR", "BSK", "RRN", "HUB", "MCC", "MYS", "RJN"],
    ...
    'Rest of India(incl. Jaipur)': ["TVM", "KTM", "KOL", "KAS", "VSH", "PUN", "HDP", "AHM", "IND", "JWS", "APR", "ATA", "KWA", "CTK", "BHU", "JPR"],
    'Kerala': ["TVM", "KTM"],
    'Kolkata': ["KOL", "KAS"],
    'Ahmedabad': ["AHM"],
  ...
  for (let key in aehgroupedBranches) {
    ...
    aehgroupedBranches[key].forEach(branch => {
      ...
      branchObj[key].push({
        ...
        ftd_cogs_percent: cogsPercent(ftd, ftdrev),
        mtdpha: mtdpha,
        mtdopt: mtdopt,
        mtdlab: mtdlab,
        mtdot: mtdot,
        ...
        mtd_cogs_percent: cogsPercent(mtd, mtdrev),
        ftdvobpha: ftdvobpha,
        ftdvobot: ftdvobot,
        ftdvobopt: ftdvobopt,
        ftdvoblab: ftdvoblab,
        ftdvobcons: ftdvobcons,
        ftdvobothers: ftdvobothers,
        ftdvob: ftdvob,
        ftd_vob_percent: cogsPercent(ftd, ftdvob),
        mtdvobpha: mtdvobpha,
        mtdvobopt: mtdvobopt,
        mtdvoblab: mtdvoblab,
        mtdvobot: mtdvobot,
        mtdvobcons: mtdvobcons,
        mtdvobothers: mtdvobothers,
        mtdvob: mtdvob,
        mtd_vob_percent: cogsPercent(mtd, mtdvob),
  ...
  ohctempObj['Mauritius'] = {
    'ftdpha': ohcBranch['EBN'].ftdpha + ohcBranch['FLQ'].ftdpha + ohcBranch['GDL'].ftdpha,
  ...
  ohctempObj['Mozambique'] = {
    'ftdpha': ohcBranch['MZQ'].ftdpha + ohcBranch['BRA'].ftdpha,
