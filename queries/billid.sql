SELECT CASE WHEN dbil.`Approval_status`=0 THEN 'Submitted' WHEN dbil.`Approval_status`=1 THEN 'SCH Approved' WHEN dbil.`Approval_status`=2 THEN 'Finance Approved' ELSE '0' END AS 'status',rdt.entity,rdt.TRANSACTION_DATE,rdt.BILLED,rdt.NATIVE,rdt.PAYORTYPE,rdt.PAYER_NAME,rdt.AGENCY_NAME,rdt.UNIT,rdt.GROUP,rdt.SUBGROUP,rdt.MRN,rdt.PATIENT_NAME,rdt.REFERRAL_TYPE,rdt.REFERRAL_BY,
       rdt.REFERRAL_VALUE,
       rdt.PATIENT_AGE,
       rdt.REGISTERED_DATE,
       rdt.VISIT_TYPE,
