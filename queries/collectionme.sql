...
revenue_details REV
WHERE
REV.TRANSACTION_DATE BETWEEN DATE_FORMAT(CURDATE() - INTERVAL 1 MONTH,'%Y-%m-01') AND LAST_DAY( CURDATE() - INTERVAL 1 MONTH)
  AND ENTITY IN ('AEH','AHC','AHI')
GROUP BY
BILLED
...
collection_detail COL
WHERE
PAYMENT_OR_REFUND_DATE BETWEEN DATE_FORMAT(CURDATE() - INTERVAL 1 MONTH,'%Y-%m-01') AND LAST_DAY( CURDATE() - INTERVAL 1 MONTH)
  AND PARENT_BRANCH IN ('AEH','AHC','AHI')
GROUP BY
BRANCH
)
