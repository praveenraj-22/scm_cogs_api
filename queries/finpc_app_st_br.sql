...
,refilled_amount,refill,sum(can) as can,Submitted_date,created_by,cancel_date,STATUS AS 'statusno',bill_submissio, CONCAT(branch,approved,cancelled,pending,bill_submission) AS 'test',IF(Payment_receipt IS NULL,'NA',Payment_receipt) AS Payment_receipt FROM
(
--
...
-- --  CONCAT(branch,approved,cancelled,pending,bill_submission) AS 'test' FROM
-- --  (
