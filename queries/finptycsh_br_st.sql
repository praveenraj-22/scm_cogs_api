SELECT * FROM (SELECT b.branch,
  ...
  ON cancelled.branch = b.branch
     ...
     ORDER BY b.STATUS DESC, Submitted_date DESC) AS c WHERE c.branch=? and c.status1=?
...
-- -- (
-- -- SELECT branch,STATUS AS 'status1' FROM pettycash
-- -- WHERE STATUS =?  AND  DATE(created_date) BETWEEN '2020-08-01' AND '2020-08-31'
-- -- GROUP BY branch
-- -- HAVING COUNT(*) >=1
-- -- )AS A ON PC.branch = A.branch
-- -- WHERE  DATE(created_date) BETWEEN '2020-08-01' AND '2020-08-31' AND PC.status=?
-- -- GROUP BY PC.branch
-- -- ) AS a
-- --
-- -- )AS b
