...
WHERE DATE(Created_on) BETWEEN ? AND ? AND Approval_status=1  AND Billed_branch = ? ORDER BY Approved_time DESC) AS a ORDER BY Billed_branch ASC )AS a1
UNION
SELECT * FROM (SELECT * FROM (SELECT   CASE
           WHEN Approval_status=0 THEN 'Need SCH Approval'
           WHEN Approval_status=1 THEN 'Pending'
...
INNER JOIN `users` AS usr ON usr.emp_id=bill.Created_by AND usr.role='ch_user'
left JOIN `users` AS schusr ON schusr.emp_id=bill.Sch_Approved_by AND schusr.role='sch_user'
WHERE DATE(Created_on) BETWEEN ? AND ? AND Approval_status=2  AND Billed_branch = ? ORDER BY Admin_Approved_time DESC ) AS b ORDER BY Billed_branch ASC) AS b1
UNION
...
INNER JOIN `users` AS usr ON usr.emp_id=bill.Created_by AND usr.role='ch_user'
left JOIN `users` AS schusr ON schusr.emp_id=bill.Sch_Approved_by AND schusr.role='sch_user'
WHERE DATE(Created_on) BETWEEN ? AND ? AND Approval_status=4 AND Billed_branch = ? ORDER BY Approved_time DESC
...
INNER JOIN `users` AS usr ON usr.emp_id=bill.Created_by AND usr.role='ch_user'
left JOIN `users` AS schusr ON schusr.emp_id=bill.Sch_Approved_by AND schusr.role='sch_user'
WHERE DATE(Created_on) BETWEEN ? AND ? AND Approval_status=3  AND Billed_branch = ? ORDER BY Cancelled_time DESC
...
INNER JOIN `users` AS usr ON usr.emp_id=bill.Created_by AND usr.role='ch_user'
left JOIN `users` AS schusr ON schusr.emp_id=bill.Sch_Approved_by AND schusr.role='sch_user'
WHERE DATE(Created_on) BETWEEN ? AND ? AND Approval_status=0  AND Billed_branch = ? ORDER BY Created_on DESC
