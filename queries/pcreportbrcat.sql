...
pc.fin_id,DATE(pc.fin_approved_date) AS fin_approved_date,pc.bill_submission,pc.cancel_by,pc.cancel_date
...
INNER JOIN branches AS br ON br.code=pc.branch
 WHERE DATE(pc.created_date) BETWEEN ? AND ?
 AND br.code=? AND pc.category_id=?
ORDER BY
br.code)
 AS a


-- SELECT a.*,CONCAT(entity,branch,CODE,category_name,voucher_no,vendorname,bill_no,bill_date,credit,debit) AS vendormatch FROM (
--   SELECT CASE
-- WHEN pc.status=1 THEN 'Need sch approval'
-- WHEN pc.status=2 THEN 'Sch Approved'
-- WHEN pc.status=3 THEN 'Sch cancelled'
